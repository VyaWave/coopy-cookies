...
chrome.tabs.getSelected(null, function (tab) {
    ...
    historyUrls = JSON.parse(window.localStorage.getItem('historyUrls') || '{}');

    $('#urlInput').val(historyUrls[currentHostname] || window.localStorage.getItem('targetUrl') || 'http://localhost/')
});


$('#urlInput').change(function(e) {
    window.localStorage.setItem('targetUrl', e.target.value);
})

$('#urlInput').focus(function(e) {

    var html = '<ul>';
    var keys = Object.keys(historyUrls);
    for (var i=0; i<keys.length; i++) {
        html += '<li>' + keys[i] + ' - <span>' + historyUrls[keys[i]] + '</span></li>'
    }
    html += '</ul>';

    $('#historyInput').html(html)
})
...
$('#historyInput').click(function(e) {
    var currentEle = e.target;

    var urlDom = currentEle.querySelector('span') || currentEle;

    $('#urlInput').val(urlDom.innerHTML)
})
...
$('#copyButton').click(e => {
    ...
    chrome.cookies.getAll({
        url: currentUrl
        // domain: 'http://ads.didiglobal.com'
    }, function (cookies) {

        var targetUrl = $('#urlInput').val();

        var targetHostname = new URL(targetUrl).hostname;

        historyUrls[currentHostname] = targetUrl;
        window.localStorage.setItem('historyUrls', JSON.stringify(historyUrls)) 

        var list = JSON.parse(JSON.stringify(cookies));
        for (var i=0; i<list.length; i++) {
            chrome.cookies.set({
                url: targetUrl,
                domain: targetHostname,
                expirationDate: list[i].expirationDate,
                httpOnly: list[i].httpOnly,
                name: list[i].name,
                path: list[i].path,
                sameSite: list[i].sameSite,
                secure: list[i].secure,
                value: list[i].value,
            });
        }
